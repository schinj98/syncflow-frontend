...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  useEffect(() => {
    ...
    setUserEmail(localStorage.getItem("email") || "user@syncflow.com");
  }, []);

  const menuItems = [
    { icon: <LayoutDashboard className="w-5 h-5" />, label: 'Dashboard', href: '/dashboard' },
    { icon: <User className="w-5 h-5" />, label: 'Add Product', href: '/create-listing' },
    { icon: <User className="w-5 h-5" />, label: 'Listings', href: '/listings' },
  ...
  // 🔥 Logout Function
  const handleLogout = () => {
    localStorage.clear();         // remove token, userId, userName, email
